...
import first.menu.setupFirstPageMenu
...
class FirstPageView : JFrame("ADBCard ${getCurrentVersion()}") {
    ...
    val topbarView: TopbarView by lazy { TopbarView() } // Assuming this is a JPanel or JComponent
    val topbarViewController: TopbarViewController by lazy { TopbarViewController(topbarViewModel, topbarView) }
    var onNavigationClick:((TypeOfScreens)->Unit)?=null


    init {
        layout = BorderLayout()
        setupFirstPageMenu(menuBar,{
            onNavigationClick?.invoke(it)
        })

        bodyPanel.add(packagesListView, TypeOfScreens.apps.name)
//        bodyPanel.add(jFeedbackPane, TypeOfScreens.feedback.name)
    ...
    fun updateControllerData(deviceModel: DeviceModel?) {
        packagesListController.setDeviceModel(deviceModel)
